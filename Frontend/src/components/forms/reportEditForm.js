...
const ReportEditForm = ({ reportId, user, setOpen }) => {
  ...
  useEffect(() => {
    const fetchReportData = async () => {
      try {
        const response = await axios.get(`http://localhost:5001/report/${reportId}`);
        const reportData = response.data; 
        setFormData(reportData?.report); 
      } catch (error) {
        console.error(error);
      }
    };
  ...
  const handleSubmit = (event) => {
    ...
    if (formData.testType === 'Cancer Screening Test') {
      ...
      payload.testResults['Non-HDL Cholesterol'] = parseFloat(formData.nonHDLCholesterol);
      ...
      payload.testResults['Triglycerides/HDL Ratio'] = parseFloat(formData.triglyceridesHDLRatio);
      payload.testResults['HDL to LDL Ratio'] = parseFloat(formData.HDLLDLRatio);
    }
  
    axios
      .put(`http://localhost:5001/report/${reportId}`, payload)
      .then((res) => {
        Swal.fire({
          icon: 'success',
